function Button (props)
{
    ...
    return (
        <button onClick={() => alert('ki, love you~')} disabled={props.disabled} className={styling}><span className='material-icons'>{props.startIcon}</span><span>{names}</span><span className='material-icons'>{props.endIcon}</span></button>
    )
}
